...
struct additional_edges {
  ...
  void connect(node* a, node* b) {
    ...
    edges_.emplace_back(std::make_unique<edge>(make_edge(edge_info_, a, b, d)));
    auto const* a_to_b = edges_.back().get();
    edges_.emplace_back(std::make_unique<edge>(make_edge(edge_info_, b, a, d)));
  ...
  edge_info_idx_t edge_info_{0};  // edge info created during preprocessing
};
